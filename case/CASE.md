# CASE

> `/case` doldurur. Ham sayfa metinleri `case/pages/` altındadır — çelişkide **ham metin hakemdir.**

## Triyaj (ilk 10 dakika — bunlar bilinmeden kod yazılmaz)

| # | Soru | Cevap | Kaynak |
|---|---|---|---|
| 1 | Code competition mı (notebook submission)? | | |
| 2 | Metrik ve yönü (büyük iyi / küçük iyi)? | | |
| 3 | Submission formatı (kolonlar, sıra, satır sayısı)? | | |
| 4 | Günlük gönderim limiti? | | |
| 5 | İnternet / süre / GPU kısıtı? | | |
| 6 | Veri boyutu ve dosyaları? | | |
| 7 | Katılım ve kural kabulü yapıldı mı? | | |
| 8 | Eşzamanlı koşu limiti (ölçüldü mü)? | | |
| 9 | Bitiş saati ve final seçim hakkı? | | |
| 10 | Dış veri / hazır model izni? | | |

## Görev
- Girdi → tahmin birimi → çıktı:
- Hedef değişken:
- ID kolonu:
- Grup / zaman kolonu:

## Ürün bölümü (15 dk — Onay 1'i bekletmez)
- Kullanıcı kim:
- Tahmin hangi kararı destekliyor:
- Gerçek kullanımda hangi girdiler olacak:
- Model nerede başarısız olur:

> Dikkat: sadece yarışma dosyasının toplu istatistiklerinden türeyen feature'lar
> (global mean, target encoding vb.) yeni bir kullanıcı girdisinde nasıl üretilecek?
> KABUL anında `card.md`'ye tek satır not düşülür.

## Jüri / değerlendirme
- Kaggle skoru dışındaki kriterler:
- Ağırlıkları:
- İkinci aşamada beklenen çıktı:

## Açık sorular / engeller
- 
